...
export default function PaymentScreen() {
  ...
  const bookingData = JSON.parse(
    (useLocalSearchParams().bookingData as string) || "{}"
  );
  const property = getPropertyById(id as string);
  const [paymentType, setPaymentType] = useState<"full" | "split">("full");
  const [paymentMethod, setPaymentMethod] = useState<"stripe" | "coinflow">(
    "stripe"
  );
  ...
  const payNowAmount =
    paymentType === "full" ? totalAmount : Math.round(totalAmount * 0.5);
  const remainingAmount =
    paymentType === "full" ? 0 : totalAmount - payNowAmount;
  const handlePayment = async () => {
    ...
    try {
      ...
      await createBooking({
        ...
        currency: "USD",
        paymentMethod:
          paymentMethod === "stripe" ? "Stripe Card" : "Coinflow Crypto",
      });
      toast.success(
        paymentType === "full"
      ...
      <View style={styles.actionBar}>
        <Button
          title={
            isLoading
              ? "Processing..."
              : `Proceed to Pay $${payNowAmount.toLocaleString()}`
          }
